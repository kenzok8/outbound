...
func TestTCPConnCloseClosesUnderlayBeforeCleanupLocks(t *testing.T) {
	for _, tc := range []struct {
		name string
		lock func(*TCPConn) *sync.Mutex
	}{
		{name: "read", lock: func(c *TCPConn) *sync.Mutex { return &c.readMutex }},
		{name: "write", lock: func(c *TCPConn) *sync.Mutex { return &c.writeMutex }},
	} {
		t.Run(tc.name, func(t *testing.T) {
			underlay := newCloseOrderConn()
			conn := &TCPConn{Conn: underlay}
			mu := tc.lock(conn)
			mu.Lock()
